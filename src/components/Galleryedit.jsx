...
const Galleryedit = () => {
  ...
  useEffect(() => {
    const fetchGallery = async () => {
      try {
        const response = await fetch("https://dewc-vercel.vercel.app/getgallery", {
          method: "GET",
          headers: {
  ...
  const handleDelete = (_id) => {
    fetch(`https://dewc-vercel.vercel.app/deletegallery/${_id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((res) => res.json())
      .then((data) => {
        const updatedGallery = galleries.filter(
  ...
  const handleUpload = () => {
    ...
    const uploadPromises = Array.from(files).map((file) => {
      ...
      return fetch("https://api.cloudinary.com/v1_1/dvad0qi7r/image/upload", {
        method: "POST",
        body: formData,
      })
    ...
    Promise.all(uploadPromises)
      .then((imageUrl) => {
        ...
        postGallery(imageUrl);
      })
      .catch((error) => console.error("Error uploading image:", error));
  };

  const postGallery = (imageUrl) => {
    fetch("https://dewc-vercel.vercel.app/creategallery", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
